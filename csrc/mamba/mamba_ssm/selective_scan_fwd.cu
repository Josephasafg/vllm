...
void selective_scan_fwd_kernel(SSMParamsBase params) {
    ...
    const int* chunk_start_offsets = params.chunk_start_offsets_ptr != nullptr ?
                                     reinterpret_cast<const int*>(params.chunk_start_offsets_ptr) : nullptr;
    ...
    // Chunk alignment for prefix caching:
    // chunk_start_offset is (num_computed_tokens % block_size) - where we start within the first block
    ...
    const int first_chunk_size = (chunk_start_offset > 0) ?
                                  min(seqlen, block_size - chunk_start_offset) :
                                  min(seqlen, block_size);
    ...
    for (int chunk = 0; chunk < n_chunks; ++chunk) {
        ...
        for (int r = 0; r < kNRows; ++r) {
            ...
            // Use chunk_tokens instead of seqlen - chunk * kChunkSize for aligned chunking
        ...
        // Advance by actual chunk_tokens, not kChunkSize
        u += chunk_tokens;
        delta += chunk_tokens;
        ...
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            ...
            for (int r = 0; r < kNRows; ++r) {
                ...
                for (int i = 0; i < kNItems; ++i) {
                    ...
                    // Zero out elements beyond chunk_tokens for correct state computation
                ...
                if (threadIdx.x == 0) {
                    ...
                    if (params.cache_enabled && batch_cache_indices != nullptr) {
                        // After this chunk, we'll be at position: current_position + chunk_tokens
                        // The block index we just completed is: (current_position + chunk_tokens - 1) / block_size
                        // But for the last chunk, we use block_idx_last_scheduled
                        ...
                        size_t cache_slot;
                        if (chunk == n_chunks - 1) {
                            // Last chunk: use block_idx_last_scheduled
                            cache_slot = batch_cache_indices[block_idx_last_scheduled[batch_id]];
                        } else {
                            // Intermediate chunk: write to the block we just completed
        ...
        // Use tokens_processed for output pointer offset (not chunk * kChunkSize)
        ...
        // Advance Bvar/Cvar by actual chunk_tokens
        Bvar += chunk_tokens;
        Cvar += chunk_tokens;
...
void set_ssm_params_fwd(SSMParamsBase &params,
    ...
    params.chunk_start_offsets_ptr = chunk_start_offsets.has_value() ? chunk_start_offsets.value().data_ptr() : nullptr;
