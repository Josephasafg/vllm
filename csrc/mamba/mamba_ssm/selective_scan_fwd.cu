...
void selective_scan_fwd_kernel(SSMParamsBase params) {
    ...
    // If cu_chunk_seqlen is provided (APC mode), read pre-computed chunk metadata.
    // Otherwise, fall back to simple chunking with block_size.
    ...
    if (cu_chunk_seqlen != nullptr && last_chunk_indices != nullptr) {
        ...
        // Derive absolute position from block_idx_first and first chunk size
        const int first_chunk_tokens = cu_chunk_seqlen[first_chunk_idx + 1] - cu_chunk_seqlen[first_chunk_idx];
        // chunk_start_offset = block_size - first_chunk_tokens when first chunk is partial,
        // but we can compute current_position directly:
        // If first chunk fills remainder of a block: offset = block_size - first_chunk_tokens
        // If first chunk is a full block (or seqlen < block_size with offset=0): offset = 0
        // In both cases: current_position = block_idx_first * block_size + offset
        // We detect partial first chunk: it's partial if first_chunk_tokens < block_size AND n_chunks > 1
        // OR if seqlen == first_chunk_tokens AND seqlen < block_size (single short chunk)
    ...
    for (int chunk = 0; chunk < n_chunks; ++chunk) {
        int chunk_tokens;
        if (cu_chunk_seqlen != nullptr) {
            const int chunk_idx = first_chunk_idx + chunk;
            chunk_tokens = cu_chunk_seqlen[chunk_idx + 1] - cu_chunk_seqlen[chunk_idx];
        } else {
